...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def pushButton_click(self):
        ...
        self.table_model.updateData(data)
    ...
    def pushButton_5_click(self):
        load_by_data = """select * from shipments WHERE date='{}'""".format(self.dateEdit_2.dateTime().toString("yyyy-MM-dd"))
    ...
    def button_add_click(self):
       ...
       # '\"2020-10-10\",' + '\"' + self.combo_base.currentText() + '\",\"' + self.edit_abs_num.text() + '\",\"' \
        ...
        VALUES ("{}","{}","{}","{}","{}","{}","{}","{}")
		'''.format(self.dateEdit.dateTime().toString("yyyy-MM-dd"), self.combo_base.currentText(), self.edit_abs_num.text(), 
        self.combo_client.currentText(), self.edit_volume.text(), self.combo_type_1.currentText(), self.combo_type_2.currentText(),
        self.edit_cost.text())

        self.db_work_obj.insertnewdata(new_add_data)
        print(new_add_data)
    ...
    def button_change_click(self):
        ...
        WHERE id_shipment = '{}'
		'''.format(self.dateEdit_change.dateTime().toString("yyyy-MM-dd"), self.combo_base_change.currentText(), self.edit_abs_num_change.text(), 
        self.combo_client_change.currentText(), self.edit_volume_change.text(), self.combo_type_1_change.currentText(), self.combo_type_2_change.currentText(),
        self.edit_cost_change.text(), self.id_edit_row)
        self.db_work_obj.insertnewdata(new_change_data)
    ...
    def fill_table(self, data):
        for row in data:   
            ...
            for i, column in enumerate(row):
                self.tableView.setItem(rowPosition, i, QtWidgets.QTableViewItem(str(column)))
...
class MyTableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, parent, mylist, header, *args):
        ...
        mylist2 = [(mydata[0], mydata[1], mydata[2], mydata[3], mydata[4], mydata[5], mydata[6], mydata[7] ,mydata[8], float(mydata[5]) * float(mydata[8]) ) for mydata in mylist]
        self.mylist = mylist2
        self.header = header
    ...
    def updateData(self, data):
        ...
        mylist2 = [(mydata[0], mydata[1], mydata[2], mydata[3], mydata[4], mydata[5], mydata[6], mydata[7] ,mydata[8], float(mydata[5]) * float(mydata[8]) ) for mydata in data]
        self.mylist = mylist2
        self.endResetModel()